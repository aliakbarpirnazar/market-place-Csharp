...
namespace ServiceLayer.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        {
            var userseller = _context.Users.Where(x => x.PhoneNumber == phoneNumber).Include(x=>x.Seller).FirstOrDefault();
         
            if(userseller.SellerId == null)
            {
                return _context.Users.Where(x => x.PhoneNumber == phoneNumber)
            ...
            {
                if(userseller.Seller.Request == 2)
                {
                    return _context.Users.Where(x => x.PhoneNumber == phoneNumber)
            .Select(x => new UserPanelSideBarViewModel
            {
